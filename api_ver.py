...
def get_file_info(file, filename):
    ...
    if extention == 'zip':
        ...
        ac_dict, re_dict = customZip_ner(candis_df, name, email, phone, in_, gender)
...
def custom_ner(data):
    ...
    if jd_file_.endswith('.docx'):
        ...
        if job_desc.loc[0, "Total Experience"] <= emp_df.loc[0, "Total Experience"]:
            ...
            print(ac_df)
            print(re_df)
            ac_df = ac_df[['index', 'FirstName', 'LastName', 'Gender', 'Email id', 'Phone No', 'Similarity', 'Remark']]
            re_df = re_df[['index', 'FirstName', 'LastName', 'Gender', 'Email id', 'Phone No', 'Similarity', 'Remark']]
...
def customZip_ner(candis_df, name, email, phone, mk, gender):
    jd_file_ = 'JD/Java_Developer.docx'
    if jd_file_.endswith('.docx'):
        jd_file = jd_file_
        job_dict = {}
        ...
        cos_df = cos_df[['index', 'Name', 'Gender', 'Similarity', 'Status', 'Remark', 'Email id', 'Phone No']]
